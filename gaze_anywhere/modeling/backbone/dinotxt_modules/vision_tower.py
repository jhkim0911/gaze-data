...
from ..layers import SelfAttentionBlock, SwiGLUFFN, named_apply
from ..dinov3 import init_weights_vit
...
class VisionTower(nn.Module):
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        return torch.cat(features, dim=-1), class_token, patch_tokens, backbone_patch_tokens
...
def build_vision_model(
    ...
    if backbone is None:
        if backbone_model_config is not None:
            raise RuntimeError(
                "Building vision backbone from config is not supported. Please provide a backbone directly."
            )
        else:
            raise RuntimeError(
